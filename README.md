...
requests
 возвращает объект класса Response, который позволяет получить содержимое ответа, а также убедиться, что запрос отработал без ошибок, с помощью кода статуса ответа.
